...
class barker_generator:
    def __init__(self):
        pass

    def validate_barker(self,sequence):
        l = len(sequence)
        if l < 2:
            return 
    ...
    def binary_levelshifted(self,x):
        y = list("{0:b}".format(x))
    ...
    def find_barker(self,length):
        valid_barkers = []
        for i in range(2**length-1,int(2**(length-1))-1,-1):
            attempt = self.binary_levelshifted(i)
            return_code = self.validate_barker(attempt)  
            if(return_code):
    ...
    def find_all_barker(self,max_length,*argv):
        legalize = False
        ...
        for i in range(min_length,max_length+1):
            barker_sequences = self.find_barker(i)
            if(barker_sequences):
                if(legalize):
                    barker_sequences=(self.legalize(barker_sequences))
                #print("======================================")
                #print("length = " + str(i))
                for sequence in barker_sequences:
                    pass
                    #print(sequence)
            return barker_sequences

if __name__ == "__main__":
    barker_gen = barker_generator()
    target_length = 0
    while(target_length<2):
        target_length = int(input('Please enter the target barker code length\r\n'))
    ...
    for arg in sys.argv[1:]:
        if "a" in arg:
            barker_gen.find_all_barker(target_length,'a')
            exit()  
    barker_gen.find_all_barker(target_length)
